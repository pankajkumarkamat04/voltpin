    ...
    <div className="min-h-screen flex flex-col bg-[#F8F8F8] pb-20 relative">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto py-4">
          {[
            ...
            { icon: 'games', label: 'Games', href: '/home' },
        ...
        <div className="p-6 border-t border-gray-200">
          <button className="w-full bg-gradient-to-r from-[#2F6BFD] to-[#2563eb] text-white py-3.5 rounded-xl font-semibold text-base shadow-md active:opacity-90 transition-opacity touch-manipulation">
            Log Out
          </button>
        </div>
      </div>
      ...
      <header className="bg-[#2F6BFD] px-4 py-3 flex items-center justify-between z-50 relative">
        ...
        <div className="flex items-center gap-2">
          {/* Points Button */}
          <button className="bg-white hover:bg-gray-50 rounded-full px-3 py-1.5 flex items-center gap-1.5 touch-manipulation shadow-md">
            <Image
              src="/coin.png"
              alt="Coin"
              width={16}
            ...
            />
            <span className="text-gray-900 font-semibold text-sm">500</span>
          </button>
          {/* Profile Icon */}
      ...
      <main className="flex-1 px-4 pb-4">
        ...
        <div className="mb-6 bg-[#2F6BFD] -mx-4 px-4 pt-4 pb-6">
          <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-2 snap-x snap-mandatory scroll-smooth">
            {[1, 2, 3].map((item, index) => (
              <div
                key={item}
                className={`flex-shrink-0 w-[85%] sm:w-[90%] bg-white rounded-2xl shadow-md h-48 snap-center ${
                  index === 0 ? 'ml-0' : ''
                }`}
              />
            ))}
        ...
        <div className="grid grid-cols-4 gap-2 mb-6">
          {[
            { icon: HiPlus, label: 'Add Points' },
            { icon: HiShoppingBag, label: 'Orders' },
            { icon: HiChartBar, label: 'Leaderboards' },
            { icon: HiPaperAirplane, label: 'Contact Us' },
          ].map((item, index) => {
        ...
        <div className="flex items-center gap-2 mb-4">
          {/* All Filter Button */}
          <button className="bg-[#2F6BFD] text-white px-3 py-2 rounded-full flex items-center gap-1 text-xs font-medium touch-manipulation flex-shrink-0">
            <span>All</span>
          ...
          {/* Search Button */}
          <button className="bg-[#2F6BFD] text-white p-2 rounded-lg touch-manipulation flex-shrink-0">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[1, 2, 3, 4, 5, 6].map((item) => (
            ...
            >
              ...
              {/* Game Info */}
              <div className="p-3">
                <h3 className="text-gray-900 font-semibold text-sm mb-1">Mobile Legends</h3>
                <p className="text-gray-500 text-xs">Moonton</p>
              </div>
            </div>
          ))}
        </div>
